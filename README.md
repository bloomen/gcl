...
A tiny graph concurrent library for C++. Requires a C++14 compliant compiler. Tested with Clang, GCC, and Visual Studio.

Sample usage:
```cpp
auto t1 = gcl::task([]{ return 42; });
auto t2 = gcl::task([]{ return 13.3; });
auto t3 = gcl::task([](auto t1, auto t2){ return t1.get() + t2.get(); }, t1, t2);
gcl::Async async{4};
t3.schedule(async);
